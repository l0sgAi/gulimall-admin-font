        ...
        <el-col :span="20">
            <div class="mod-product__attr">
                ...
                <el-table ref="attrTable" v-loading="state.dataListLoading" :data="state.dataList" border
                    ...
                    <el-table-column prop="searchType" header-align="center" align="center" label="可检索" width="90">
                        <template #default="scope">
                            <el-tag v-show="scope.row.attrType == '0'" type="info">不支持</el-tag>
                            <el-switch v-if="scope.row.attrType != '0'" v-model="scope.row.searchType" class="ml-2"
              ...
              <template #default="scope">
                ...
                <el-tag v-else>多选</el-tag>
                    ...
                    <el-table-column prop="enable" header-align="center" align="center" label="启用" width="70">
                        <template #default="scope">
                            <el-switch v-model="scope.row.enable" class="ml-2" inline-prompt
                                ...
                                inactive-text="否" :active-value="1" :inactive-value="0" disabled />
                        </template>
                    </el-table-column>
                    <el-table-column prop="catelogName" header-align="center" align="center" label="所属分类"
                        #default="scope">
                        <el-cascader placeholder="无" v-model="scope.row.catelogId" :options="treeDataRef"
                            :props="casProps">
                        </el-cascader>
                    </el-table-column>
                    <el-table-column prop="groupName" header-align="center" align="center" label="所属分组"
                        #default="scope">
                        <el-tag v-show="scope.row.attrType == '0'" type="info">不支持</el-tag>
                        <el-cascader v-if="scope.row.attrType != '0'" placeholder="无" v-model="scope.row.groupName"
                    ...
                    <el-table-column v-if="attrtype == 1" prop="showDesc" header-align="center" align="center"
                        ...
                        <template #default="scope">
                            <el-tag v-show="scope.row.attrType == '0'" type="info">不支持</el-tag>
                            <el-switch v-show="scope.row.attrType != '0'" v-model="scope.row.showDesc" class="ml-2"
                                inline-prompt style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949"
                                active-text="是" inactive-text="否" :active-value="1" :inactive-value="0" disabled />
                        </template>
                    </el-table-column>
...
const view = reactive({
    deleteIsBatch: true,
    getDataListURL: "/product/attr/sale/page",
    getDataListIsPage: true,
    exportURL: "/product/attr/sale/export",
    deleteURL: "/product/sale/attr"
});
...
const getNodeHandle = (node: any) => {
    ...
    if (node.catLevel === 3) {
        baseService.get(`product/attr/sale/page/${node.catId}`).then((res) => {
...
const query = () => {
    baseService.get(`product/attr/sale/page/${curCatId.value}?key=${queryKey.value}`).then((res) => {
